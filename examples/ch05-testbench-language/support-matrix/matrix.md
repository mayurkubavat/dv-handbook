...
| static properties and methods | runs | runs |
...
: What each open simulator does with the constructs this chapter teaches, measured by running one probe per row with the flags the book's build uses and checking the printed values. Of 28 constructs, 13 run on both, 10 on Verilator only, 1 on Icarus only and 4 on neither. Regenerated from the probes on every run. {#tbl-ch05-support}
