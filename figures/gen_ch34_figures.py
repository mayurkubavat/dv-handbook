...
def fig_model():
    ...
    slv = d.tool(310, 520, "Constraint solver", w=280, sub="seeded: picks a point")
    st = d.store(650, 520, "Stimulus", w=200, sub="one transaction")
    d.arrow(cons, slv); d.arrow(slv, st)
    d.arrow(st, cons, label="next transaction", sides=("b", "b"), bend=(0, 110), label_dy=30)
...
def fig_evaluation():
    ...
    d.label(40, 555, "Visible: coverage (a proxy).   Hidden: bug-finding power (mutants killed).\nTwo different experiments on small designs: testbench coverage near 90 percent (Ye et al. 2025);\ntest-plan bug detection near 14 percent (Kochar et al. 2026). Not one ratio.", 15, INK)
...
def fig_flow(delegation=False):
    ...
    for s in (spec, plan, rtl, logs): d.arrow(s, pl, sides=("b", "t"))
    ...
    d.arrow(logs, roles["Debug"], sides=("b", "t"), bend=(120, 120))
    ...
    d.arrow(roles["Formal"], g3, kind="judgment", sides=("r", "t"), bend=(160, 120), label="proof status", label_dy=-20)
